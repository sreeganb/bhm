...
from pair_score import ScoringSystem  # Assuming you have your ScoreCalculator defined
...
class PerturbSystemParameters:
    def __init__(self):
        ...
        # Fix the values of sigma and sigma_range for all the calculations 
        fixed_sigma = {'AA': 0.2025, 'AB': 0.5, 'BC': 0.5, 'CC': 0.2822}
        #fixed_sigma = {'AA': 5.0, 'AB': 1.5, 'BC': 1.5, 'CC': 5.0}
        self.sig_range = {"AA": (0.01, 10.0), "AB": (0.01, 10.0), "BC": (0.01, 10.0), "CC": (0.01, 10.0)}

        # use tetramer scorer to get the tetramers list for the ideal configuration
        self.tscorer = TetramerScorer()  # Initialize TetramerScorer
        self.tets = self.tscorer.get_tetramers(self.ideal_coordinates)  # Get tetramers
        
        self.ideal_score, _, _, _ = self.tscorer.neg_log_posterior(positions=self.ideal_coordinates,tetramers=self.tets,
                                                        sig=fixed_sigma,exclusion_weight=1.0,pair_weight=1.0,
                                                        tetramer_weight=1.0,debug=True)
        
        #self.ideal_score, _, _, _ = self.scalc.calculate_score(self.ideal_coordinates, fixed_sigma, self.sig_range) # Calculate ideal score once
    ...
    def analyze_perturbations(self, magnitudes, num_perturbations=10, seed=None):
        ...
        for mag in magnitudes:
            for _ in range(num_perturbations):
                ...
                score, _, _, _ = self.scalc.calculate_score(perturbed_coords, fixed_sigma, self.sig_range)
