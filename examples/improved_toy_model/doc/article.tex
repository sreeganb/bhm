...
\hline
\textbf{Data Type} & \textbf{Information Provided} & \textbf{Synthetic Generation Parameters} & \textbf{Likelihood Function $p(D|X)$} & \textbf{Hierarchical Parameters to Infer} \\
\hline
Distance Restraints & Average pairwise distances between particle types. & Mean distance $\mu$, Uncertainty $\sigma$. & Gaussian: $\mathcal{N}(d_{obs} | d_{calc}(X), \sigma^2)$. & $\sigma_{AA}, \sigma_{AB}, \sigma_{BC}$ (drawn from a shared hyperprior). \\
\hline
XL-MS & Topological proximity; indicates pairs of particles within a certain distance. & Cross-linker length, true positive rate $p_{true}$, false positive rate $p_{false}$. & Product of Bernoulli/Binomial likelihoods based on distance threshold. & $p_{true}$, $p_{false}$. \\
\hline
Cryo-EM Map & Low-resolution global shape and volume of the assembly. & Target resolution, Noise level, Voxel size. & Gaussian based on Cross-Correlation (CC): $\propto \exp(k \cdot \text{CC})$. & Map scale factor $k$, map uncertainty $\sigma_{map}$. \\
\hline
SAXS Profile & Ensemble-averaged shape and size in solution. & $q$-range (scattering angle), Experimental error model. & Chi-squared likelihood: $\propto \exp(-\chi^2/2)$. & Overall scale factor, constant background term. \\
\hline
\end{tabular}
\caption{A Compendium of Synthetic Data Types and Likelihood Functions}
