...
class MCMCSampler:
    ...
    def __init__(self, root_hier, dof, temperature, num_steps,  
                 output_objects, rmf_output_objects, stat_file_name_suffix, file_name="bhm0.rmf3",
                 global_output_directory="./", initial_rmf_name_suffix="initial",
                 nframes_write_coordinates=1, do_clean_first=True, 
                 do_create_directories=True, atomistic=False, test_mode=False, 
                 use_nestor=False):
    ...
    def setup_rmf_files(self):
        ...
        output.init_rmf(init_suffix + ".rmf3", output_hierarchies, listofobjects=self.rmf_output_objects)
        ...
        rmfname = globaldir + self.file_name + ".rmf3"
        output.init_rmf(rmfname, output_hierarchies, listofobjects=self.rmf_output_objects)
        if self._rmf_restraints:
    ...
    def run_mcmc(self, dof, temperature):
        ...
        output = IMP.pmi.output.Output(atomistic=self.vars["atomistic"])
        rmfname = self.vars["global_output_directory"] + "/" + self.file_name
        ...
        for i in range(nframes):
            ...
            if i % self.vars["nframes_write_coordinates"] == 0:
                print('--- writing coordinates to', rmfname)
